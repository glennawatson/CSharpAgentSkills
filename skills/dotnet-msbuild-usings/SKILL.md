...
The SDK writes these into a generated `obj/**/<Project>.GlobalUsings.g.cs` — read that file when you need to see exactly what a project's implicit usings resolved to, rather than guessing from the `.csproj`.
...
The same technique works across a two-project build: one `Shared.cs` (linked via `<Compile Include>` into both leaves) referencing unqualified `IEngine`, `Engine`, `Signal`, `Box<T>`; a shared `Seam.props` aliasing `IEngine`/`Engine`/`Signal` to `OldLib.*` for one leaf and to `NewLib.*` (plus `DefineConstants SHIM`) for a leaf whose project name ends in `New`, with `Box<T>` handled by `<Using Include="OldLib.Generics" />`/`NewLib.Generics` namespace imports since it's generic. Both leaves build and run unmodified, each resolving `Worker.Describe()` against its own library's `Engine.Run()`.
